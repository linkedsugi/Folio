...
export function AdminClient() {
  ...
  if (!hydrated) {
    return (
      <AppShell app={null} stage="start">
        <p className="py-16 text-center text-[15px] text-ink-muted">불러오는 중…</p>
      </AppShell>
    );
  }
  ...
  if (!session) {
    return (
      <AppShell app={null} stage="start">
        <div className="mx-auto max-w-md rounded-sm border border-rule bg-canvas px-6 py-8 shadow-card">
          <h1 className="text-lg font-bold text-ink">관리자 화면</h1>
          <p className="mt-2 mb-6 text-[14px] leading-relaxed text-ink-muted">
  ...
  return (
    <AppShell app={null} stage="start">
      <div className="space-y-10">
        <header>
          <p className="text-[12px] font-semibold tracking-wide text-brand">ADMIN</p>
          <h1 className="mt-1.5 text-xl font-bold text-ink sm:text-2xl">관리자 화면</h1>
        ...
        <section className="space-y-6">
          <header className="border-b border-rule pb-4">
            <p className="text-[12px] font-semibold tracking-wide text-brand">계정</p>
            <h2 className="mt-1 text-lg font-bold text-ink sm:text-xl">회원 관리</h2>
          ...
          {storeKind === "local" ? (
            <div className="rounded-sm border border-warn-soft bg-warn-soft px-5 py-4">
              <p className="text-[15px] font-bold text-warn">이 목록은 이 브라우저에만 있습니다</p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input
              ...
              className="min-w-0 flex-1 rounded-sm border border-rule-strong bg-canvas px-4 py-2.5 text-[14px]"
            />
            <button
              type="button"
              onClick={() => void refreshMembers()}
              className="rounded-sm border border-rule-strong bg-canvas px-4 py-2.5 text-[13px] font-medium text-ink hover:border-ink"
            >
          ...
          {filtered.length === 0 ? (
            <p className="rounded-sm border border-dashed border-rule-strong px-5 py-8 text-center text-[14px] text-ink-muted">
              {members.length === 0
                ? "아직 로그인한 회원이 없습니다."
                : "검색 결과가 없습니다."}
            </p>
          ) : (
            // 글자와 셀 여백을 키운 만큼 min-w 도 함께 올린다. 좁은 화면에서는
            // overflow-x-auto 가 표만 가로로 흐르게 받아, 본문이 밀려나지 않는다.
            <div className="overflow-x-auto rounded-sm border border-rule">
              <table className="w-full min-w-[56rem] border-collapse text-left">
                <thead>
                  <tr className="bg-ink text-white">
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">회원</th>
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">권한</th>
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">상태</th>
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">가입</th>
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">최근 로그인</th>
                    <th scope="col" className="px-4 py-3 text-center text-[12px] font-semibold">방문</th>
                    <th scope="col" className="px-4 py-3 text-[12px] font-semibold">관리</th>
                  </tr>
                </thead>
...
function Stat({ label, value, tone }: { label: string; value: number; tone?: "warn" }) {
  return (
    <div
      className={`rounded-sm border px-4 py-4 shadow-card ${tone === "warn" ? "border-warn-soft bg-warn-soft" : "border-rule bg-canvas"}`}
    >
      <p className={`text-[12px] font-semibold ${tone === "warn" ? "text-warn" : "text-ink-faint"}`}>
        {label}
      </p>
      <p className={`tabular mt-2 text-2xl leading-none font-bold ${tone === "warn" ? "text-warn" : "text-ink"}`}>
        {value}
      </p>
    </div>
  );
  ...
  return (
    <>
      <tr className="align-top odd:bg-canvas even:bg-surface">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[14px] font-medium text-ink">
                {member.name}
                {isSelf ? <span className="ml-1 text-[12px] text-ink-faint">(나)</span> : null}
              </p>
        ...
        <td className="px-4 py-3.5">
          {locked ? (
            <span className="rounded-sm bg-ink px-2 py-1 text-[12px] font-semibold text-white" title={ROLE_NOTE[member.role]}>
            ...
            <select
              ...
              className="rounded-sm border border-rule-strong bg-canvas px-2 py-1.5 text-[13px]"
            >
        ...
        <td className="tabular px-4 py-3.5 text-[13px] text-ink-muted">{member.joinedAt.slice(0, 10)}</td>
        <td className="tabular px-4 py-3.5 text-[13px] text-ink-muted">
          {member.lastSeenAt.slice(0, 10)}
        </td>
        <td className="tabular px-4 py-3.5 text-center text-[13px] text-ink-muted">{member.visits}</td>
        ...
        <td className="px-4 py-3.5">
          <div className="flex flex-wrap gap-1">
            <button
              ...
              className="rounded-sm border border-rule-strong px-2 py-1 text-[12px] text-ink-muted hover:border-ink hover:text-ink"
            >
              메모
            </button>
            {!locked ? (
              <>
                <button
                  ...
                  className="rounded-sm border border-rule-strong px-2 py-1 text-[12px] text-ink-muted hover:border-warn hover:text-warn"
                >
                ...
                {confirming ? (
                  <span className="flex items-center gap-1">
                    <button
                      type="button"
                      onClick={onRemove}
                      className="rounded-sm bg-danger px-2 py-1 text-[12px] font-medium text-white"
                    >
                      정말 삭제
                    </button>
                    <button
                  ...
                  <button
                    ...
                    className="rounded-sm border border-rule-strong px-2 py-1 text-[12px] text-ink-muted hover:border-danger hover:text-danger"
                  >
                    삭제
                  </button>
                )}
      ...
      {editingNote ? (
        <tr className="bg-surface-sunken">
          <td colSpan={7} className="px-4 py-4">
            <label htmlFor={`note-${member.id}`} className="text-[12px] font-semibold text-ink">
              운영 메모 — {member.name}
            </label>
            ...
            <textarea
              ...
              rows={2}
              maxLength={500}
              className="mt-1 w-full rounded-sm border border-rule-strong bg-canvas px-3 py-2 text-[13px]"
            />
            <div className="mt-1.5 flex gap-2">
              <button
                type="button"
                onClick={onSaveNote}
                className="rounded-sm bg-ink px-3.5 py-1 text-[13px] font-medium text-white"
              >
                저장
              </button>
              <button
                ...
                className="rounded-sm border border-rule-strong px-3.5 py-1 text-[13px] text-ink-muted hover:border-ink hover:text-ink"
              >
                닫기
              </button>
            </div>
        ...
        member.note && (
          <tr className="bg-surface">
            <td colSpan={7} className="px-4 pb-3 text-[12px] leading-relaxed text-ink-muted">
              메모: {member.note}
            </td>
          </tr>
        )
