    ...
    <section className="rounded-sm border border-rule bg-canvas">
      <div className="border-b border-rule px-5 py-4 sm:px-6">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-[12px] font-semibold tracking-wide text-brand">지금의 판단</span>
          <span className="rounded-sm bg-ink px-3 py-1 text-[13px] font-bold text-white">
            {VERDICT_LABEL[verdict]}
          </span>
        </div>
        <p className="mt-2.5 text-[15px] leading-relaxed text-ink">{note}</p>
      ...
      <div className="px-5 py-4 sm:px-6">
        <p className="text-[14px] font-bold text-ink">필수 조건</p>
        <p className="mt-1 text-[12px] leading-snug text-ink-faint">
          전체 매칭률과는 별개의 정보입니다. 총점이 올라도 필수 조건 충족을 뜻하지 않습니다.
        </p>
        {mustHave.length === 0 ? (
          <p className="mt-3 text-[14px] text-ink-muted">공고에서 필수 조건을 찾지 못했습니다.</p>
        ) : (
          <ul className="mt-3 divide-y divide-rule border-y border-rule">
            {mustHave.map((m) => (
              <li key={m.requirementId} className="flex flex-wrap gap-x-3 gap-y-1.5 py-3">
                <span
                  className={`h-fit shrink-0 rounded-sm px-2 py-1 text-[12px] font-semibold ${STATE_TONE[m.state]}`}
                >
                  {MUST_HAVE_STATE_LABEL[m.state]}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="text-[14px] leading-relaxed font-medium text-ink">{m.label}</p>
                  {m.note ? (
                    <p className="text-[13px] leading-relaxed text-ink-muted">{m.note}</p>
                  ) : null}
                </div>
              </li>
            ))}
