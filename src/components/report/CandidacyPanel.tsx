...
export function CandidacyPanel({
  ...
  return (
    ...
    >
      ...
      >
        <p
          className={`text-[11px] font-semibold tracking-wide ${future ? "text-goal" : "text-brand"}`}
        >
          {future ? "실행 과제를 마쳤다면" : "지금 지원한다면"}
        </p>
        <h3 className="mt-0.5 text-sm leading-snug font-bold text-ink">{candidacy.headline}</h3>
      </header>
          ...
          <ul className="space-y-1">
            {candidacy.reasonsToConsider.map((r, i) => (
              <li key={i} className="flex gap-1.5 text-[13px] leading-relaxed">
                <span aria-hidden className="mt-[0.55em] size-1 shrink-0 rounded-full bg-ok" />
                <span>{r}</span>
              </li>
            ))}
          </ul>
            ...
            {candidacy.concerns.map((c, i) => {
              ...
              return (
                <li key={i} className="rounded-sm border border-rule bg-surface px-3 py-2">
                  <p className="text-[13px] font-bold text-warn">{c.concern}</p>
                  ...
                  {used.length > 0 ? (
                    <ul className="mt-1.5 flex flex-wrap gap-1">
                      {used.map((e) => (
                        <li
                          key={e.id}
                          className="rounded-sm bg-canvas px-1.5 py-0.5 text-[11px] text-ink-muted"
                          title={e.summary}
                        >
                  ...
                  <p className="mt-1.5 border-t border-rule pt-1.5 text-[12px] leading-snug text-ink-muted">
                    그래도 남는 것: {c.honestLimit}
                  </p>
                </li>
              );
            })}
      ...
      <footer className="border-t border-rule bg-surface px-4 py-2.5">
        <p className="text-[11px] leading-relaxed text-ink-faint">{candidacy.caution}</p>
      </footer>
    </section>
  );
}
