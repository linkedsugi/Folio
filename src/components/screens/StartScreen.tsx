...
export function StartScreen({
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-sm border border-rule bg-canvas px-5 py-6 sm:px-8 sm:py-8">
        <p className="text-[11px] font-semibold tracking-wide text-brand">
          MINDCANVAS / ROLEFIT CANVAS
        </p>
        <h1 className="mt-2 text-2xl leading-tight font-bold text-ink sm:text-3xl">
          공고와 이력으로
          <br />
          지원 준비를 시작하세요
        </h1>
        ...
        <div className="mt-5 rounded-sm bg-brand-soft px-4 py-3">
          <p className="text-[12px] font-bold text-brand">분석은 세 단계로 진행됩니다</p>
          <ol className="mt-1.5 grid gap-1.5 text-[12px] leading-snug text-ink sm:grid-cols-3">
            <li>
              <span className="font-bold text-now">① 현재</span> — 직접 수행한 사실만으로 맞춰 보기
              <span className="block text-ink-faint">→ 이력서 1 · Baseline</span>
            </li>
            <li>
              <span className="font-bold text-story">② 스토리텔링 후</span> — 이미 가진 관련 경험까지 연결
              <span className="block text-ink-faint">→ 이력서 2 · 제출용</span>
            </li>
            <li>
              <span className="font-bold text-goal">③ 목표</span> — 부족분을 과제와 목표로
              <span className="block text-ink-faint">→ 이력서 3 · 미래(계획)</span>
            </li>
          </ol>
        </div>
        ...
        <button
          type="button"
          onClick={onStartOwn}
          className="mt-5 rounded-sm bg-ink px-5 py-2.5 text-sm font-bold text-white hover:bg-ink-strong"
        >
          내 자료로 시작
        </button>
      </section>
      ...
      {applications.length > 0 ? (
        <section>
          <h2 className="text-[15px] font-bold text-ink">진행 중인 지원</h2>
          <p className="text-[12px] text-ink-muted">지원 회사별로 문서를 따로 보관합니다.</p>
          <ul className="mt-2 divide-y divide-rule overflow-hidden rounded-sm border border-rule bg-canvas">
            {applications.map((a) => (
              <li key={a.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                ...
                <button
                  ...
                  className="shrink-0 rounded-sm px-1.5 py-0.5 text-[11px] text-ink-faint hover:text-danger"
                >
                  삭제
                </button>
              </li>
      ...
      <section>
        <h2 className="text-[15px] font-bold text-ink">샘플 5종으로 먼저 체험</h2>
          ...
          {samples.map((s) => (
            <li key={s.id}>
              <button
                ...
                className="flex h-full w-full flex-col rounded-sm border border-rule bg-canvas p-4 text-left transition-colors hover:border-ink"
              >
                <p className="text-[11px] font-semibold tracking-wide text-brand">{s.label}</p>
                <p className="mt-1 text-[13px] font-bold text-ink">{s.posting.roleTitle}</p>
                <p className="text-[11px] text-ink-muted">{s.posting.company}</p>
                <p className="mt-1.5 flex-1 text-[12px] leading-snug text-ink-muted">{s.tagline}</p>
                <p className="tabular mt-2.5 text-[13px] font-bold">
...
function Deliverable({
  ...
  return (
    <div
      className={
        tone === "ink"
          ? "rounded-sm border-2 border-ink bg-canvas px-3.5 py-3"
          : "rounded-sm border border-rule-strong bg-surface px-3.5 py-3"
      }
    >
      <p
        className={`text-[11px] font-semibold tracking-wide ${tone === "ink" ? "text-ink" : "text-ink-muted"}`}
      >
        {eyebrow}
      </p>
      <p className="mt-0.5 text-[14px] font-bold text-ink">{title}</p>
